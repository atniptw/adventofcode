...
let years = [];
if (filterYear) {
    if (fs.existsSync(path.join(srcPath, filterYear))) {
        years = [filterYear];
    } else {
...
for (const year of years) {
    ...
    const yearTestPath = path.join(testPath, year);
    ...
    for (const file of solutionFiles) {
        ...
        const solutionFile = path.join(yearSrcPath, file);
        ...
        console.log(`   ${dayNum.toString().padStart(3)} | ${solutionStatus.padEnd(8)} | ${testStatus.padEnd(4)} | ${inputStatus.padEnd(6)} | ${status}`);
    }
